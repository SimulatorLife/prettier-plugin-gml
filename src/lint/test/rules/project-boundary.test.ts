...
void test("posix boundary comparison is segment-safe", () => {
    assertEquals(LintWorkspace.Lint.services.isPathWithinBoundary("/root/sub/file.gml", "/root"), true);
    assertEquals(LintWorkspace.Lint.services.isPathWithinBoundary("/root2/file.gml", "/root"), false);
});
...
void test("boundary handling normalizes root separators", () => {
    assertEquals(LintWorkspace.Lint.services.isPathWithinBoundary("/root/sub/file.gml", "/root/"), true);
    assertEquals(LintWorkspace.Lint.services.isPathWithinBoundary(String.raw`C:\Root\sub\file.gml`, "c:/root/"), true);
    assertEquals(
        LintWorkspace.Lint.services.isPathWithinBoundary(
            String.raw`\\server\share\root\sub\file.gml`,
...
void test("boundary handling uses realpath when available and falls back for missing paths", () => {
    ...
    assertEquals(LintWorkspace.Lint.services.isPathWithinBoundary(symlinkDirectory, canonicalDirectory), true);

    const missingFile = path.join(tempRoot, "missing", "path", "file.gml");
    assertEquals(LintWorkspace.Lint.services.isPathWithinBoundary(missingFile, path.join(tempRoot, "missing")), true);
