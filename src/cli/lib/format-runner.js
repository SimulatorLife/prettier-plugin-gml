...
} from "./ignore-rules-negation-tracker.js";
import { CliUsageError, formatCliError } from "./cli-errors.js";
import { resolvePluginEntryPoint } from "./plugin-entry-point.js";
import {
    hasRegisteredIgnorePath,
    registerIgnorePath,
    resetRegisteredIgnorePaths
} from "./ignore-path-registry.js";
...
const PRETTIER_IGNORE_BASENAME = ".prettierignore";
const IGNORE_PATH = path.resolve(WRAPPER_DIRECTORY, PRETTIER_IGNORE_BASENAME);
const INITIAL_WORKING_DIRECTORY = path.resolve(process.cwd());

const FALLBACK_EXTENSIONS = Object.freeze([".gml"]);

export const ParseErrorAction = Object.freeze({
    REVERT: "revert",
    SKIP: "skip",
    ABORT: "abort"
});

export const VALID_PARSE_ERROR_ACTIONS = new Set(
    Object.values(ParseErrorAction)
);
...
export const VALID_PARSE_ERROR_ACTION_CHOICES = formatValidChoiceList(
    VALID_PARSE_ERROR_ACTIONS
);
export const VALID_PRETTIER_LOG_LEVEL_CHOICES = formatValidChoiceList(
    VALID_PRETTIER_LOG_LEVELS
);

export const PRETTIER_MODULE_ID =
    process.env.PRETTIER_PLUGIN_GML_PRETTIER_MODULE ?? "prettier";
export const UNKNOWN_ERROR_FALLBACK = "Unknown error";

export function formatExtensionListForDisplay(extensions) {
    ...
    const normalized = Array.from(
        new Set(
            normalizeStringList(rawExtensions, {
                ...
                .map((value) => coerceExtensionValue(value))
                .filter(Boolean)
        )
    );
...
export const DEFAULT_EXTENSIONS = normalizeExtensions(
    process.env.PRETTIER_PLUGIN_GML_DEFAULT_EXTENSIONS,
    FALLBACK_EXTENSIONS
);

export const DEFAULT_PARSE_ERROR_ACTION =
    normalizeEnumeratedOption(
...
export const DEFAULT_PRETTIER_LOG_LEVEL =
    normalizeEnumeratedOption(
 ...
 */
export async function resetFormattingSession(onParseError) {
...
async function revertFormattedFiles() {
    ...
    for (const [filePath, snapshot] of revertEntries) {
        try {
            ...
            console.error(
                `Failed to revert ${filePath}: ${
                    message || UNKNOWN_ERROR_FALLBACK
                }`
            );
        } finally {
...
async function resolveProjectIgnorePaths(directory) {
    ...
    const candidatePaths = directoriesToInspect.map((candidateDirectory) =>
        path.join(candidateDirectory, PRETTIER_IGNORE_BASENAME)
    );
...
async function resolveTargetStats(target, { usage } = {}) {
    try {
        ...
        const details =
            getErrorMessage(error, { fallback: UNKNOWN_ERROR_FALLBACK }) ||
...
async function resolveDirectoryIgnoreContext(directory, inheritedIgnorePaths) {
    const localIgnorePath = path.join(directory, PRETTIER_IGNORE_BASENAME);
...
async function resolveFormattingOptions(filePath) {
    ...
    try {
        ...
        const message = getErrorMessage(error, {
            fallback: UNKNOWN_ERROR_FALLBACK
        });
        console.warn(
...
export async function executeFormatCommand(command) {
...
function logNoMatchingFiles({
    ...
    const guidance = targetIsDirectory
        ? `Adjust --extensions or update your ${PRETTIER_IGNORE_BASENAME} files if this is unexpected.`
        : `Pass --extensions to include this file or adjust your ${PRETTIER_IGNORE_BASENAME} files if this is unexpected.`;
...
function buildSkippedFileDetailEntries({
    ...
    if (ignored > 0) {
        detailEntries.push(
            `ignored by ${PRETTIER_IGNORE_BASENAME} (${ignored})`
        );
    }
...
function buildSkippedDirectorySummaryMessage() {
    ...
    if (formattedSamples.length === 0) {
        return `Skipped ${ignored} ${label} ignored by ${PRETTIER_IGNORE_BASENAME}.`;
    }

    const sampleList = formattedSamples.join(", ");
    const suffix = ignored > formattedSamples.length ? ", ..." : "";
    return `Skipped ${ignored} ${label} ignored by ${PRETTIER_IGNORE_BASENAME} (e.g., ${sampleList}${suffix}).`;
}
