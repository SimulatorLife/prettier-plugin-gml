...
function normalizeManualTagEntry(entry) {
    const { name: rawName, commit } = assertPlainObject(entry, {
        errorMessage: "Manual tags response must contain objects with tag metadata."
    });
 ...
 */
export function createManualGitHubRequestDispatcher({ userAgent } = {}) {
 ...
 */
export function createManualGitHubCommitResolver({ requestDispatcher }) {
 ...
 */
export function createManualGitHubRefResolver({
    requestDispatcher,
    commitResolver
}) {
    const request = requestDispatcher?.execute;
 ...
 */
export function createManualGitHubFileClient({
    requestDispatcher,
    defaultCacheRoot,
    defaultRawRoot
}) {
