...
void describe("performance metric formatters", () => {
    void it("formats finite numbers with the configured unit", () => {
        assert.equal(formatMetricValue(1.2349, { unit: "ms" }), "1.235 ms");
    });

    void it("returns 'n/a' when the value is not finite", () => {
        assert.equal(formatMetricValue(null, { unit: "ms" }), "n/a");
        assert.equal(formatMetricValue(Number.NaN, { unit: "ms" }), "n/a");
    });

    void it("omits the unit when none is provided", () => {
