...
// Re-export utility functions needed by gpu-state-management-fixes.ts
export {
    attachFeatherFixMetadata,
    createCallExpressionTargetFixDetail,
    createFeatherFixDetail,
...
function applyFeatherFixesImpl(ast: any, opts: ApplyFeatherFixesOptions = {}) {
    ...
    Core.walkAst(ast, (node) => {
        if (
            node &&
            (Core.isFunctionDeclarationNode(node) || Core.isStructFunctionDeclarationNode(node)) &&
...
function removeBreakStatementsWithoutEnclosingLoops({ ast, diagnostic }) {
    ...
    const visit = (node, parent, property, breakableDepth, owner) => {
        ...
        if (Core.isBreakStatementNode(node) && breakableDepth === 0) {
...
function resolveWithOtherVariableReferences({ ast, diagnostic }) {
    ...
    const visit = (node, parent, property, arrayOwner, arrayProperty, context) => {
        ...
        if (insideWithOther && Core.isIdentifierNode(node)) {
        ...
        if (Core.isWithStatementNode(node) && isWithStatementTargetingOther(node)) {
...
function convertStringLengthPropertyAccesses({ ast, diagnostic }) {
    ...
    visitFeatherAST(ast, (node, parent, property) => {
        if (Core.isMemberDotExpressionNode(node)) {
...
function convertStringLiteralArgumentToIdentifier({ argument, container, index, diagnostic }) {
    ...
    if (!argument || !Core.isLiteralNode(argument) || typeof argument.value !== "string") {
        return null;
    }
...
function splitGlobalVarInlineInitializers({ ast, diagnostic }) {
    ...
    visitFeatherAST(ast, (node, parent, property) => {
        if (Core.isGlobalVarStatementNode(node)) {
...
function flagInvalidAssignmentTargets({ ast, diagnostic, sourceText }) {
    ...
    const visit = (node, parent, property, container, index) => {
        ...
        if (Core.isExpressionStatementNode(node)) {
        ...
        if (Core.isAssignmentExpressionNode(node)) {
...
function isAssignableTarget(node) {
    ...
    if (Core.isMemberDotExpressionNode(node) || Core.isMemberIndexExpressionNode(node)) {
        return true;
    }
...
function renameIdentifiersInNode(root, originalName, replacementName) {
    ...
    while (stack.length > 0) {
        ...
        if (Core.isIdentifierNode(node) && node.name === originalName) {
            if (
...
function unwrapIdentifierFromExpression(node) {
    ...
    if (Core.isParenthesizedExpressionNode(node)) {
...
function preventDivisionOrModuloByZero({ ast, diagnostic }) {
    ...
    visitFeatherAST(ast, (node) => {
        ...
        } else if (Core.isAssignmentExpressionNode(node)) {
...
function findZeroLiteralInfo(node) {
    ...
    if (Core.isUnaryExpressionNode(node)) {
        if (node.operator !== "+" && node.operator !== "-") {
            return null;
        }
...
function removeDuplicateMacroDeclarations({ ast, diagnostic }) {
    ...
    const visit = (node, parent = null, property = null) => {
        ...
        if (Core.isMacroDeclarationNode(node)) {
            const macroName = Core.getIdentifierText(node.name);
...
function rewriteInvalidPostfixExpressions({ ast, diagnostic }) {
    ...
    const visit = (node, parent = null, property = null, owner = null) => {
        ...
        if (Core.isIncDecStatementNode(node)) {
        ...
        const nextOwner =
            Core.isBlockStatementNode(node) || Core.isProgramNode(node) || Core.isSwitchCaseNode(node) ? node : owner;
...
function normalizeMultidimensionalArrayIndexing({ ast, diagnostic }) {
    ...
    visitFeatherAST(ast, (node, parent, property) => {
        if (Core.isMemberIndexExpressionNode(node)) {
...
function replaceDeprecatedConstantReferences({ ast, diagnostic }) {
    ...
    const visit = (node) => {
        ...
        if (Core.isIdentifierNode(node) && node.name === deprecatedConstant) {
...
export function markStatementToSuppressFollowingEmptyLine(statement) {
...
export function markStatementToSuppressLeadingEmptyLine(statement) {
    if (!statement || typeof statement !== "object") {
        return;
    }
...
function renameDuplicateFunctionParameters({
    ...
    const visit = (node) => {
        ...
        if (Core.isFunctionDeclarationNode(node) || Core.isConstructorDeclarationNode(node)) {
...
function getDeleteTargetName(node) {
    ...
    if (Core.isMemberDotExpressionNode(node)) {
        return Core.resolveNodeName(node.property as MutableGameMakerAstNode);
    }
...
function containsFileFindFirstCall(node) {
    ...
    if (Core.isFunctionDeclarationNode(node) || Core.isFunctionExpressionNode(node)) {
        return false;
    }

    if (Core.isCallExpressionNode(node) && Core.isIdentifierWithName(node.object, "file_find_first")) {
        return true;
    }
...
export function isCallExpression(node: unknown): node is { type: "CallExpression"; object?: unknown } {
    return (
        !!node &&
...
export function hasOnlyWhitespaceBetweenNodes(previous, next, sourceText) {
    if (typeof sourceText !== "string") {
        return true;
    }
...
function canWrapOperandWithReal(node) {
    ...
    if (Core.isCallExpressionNode(node)) {
        const object = node.object;
        if (object && Core.isIdentifierNode(object) && object.name === "real") {
            return false;
        }
    }
...
function containsGpuPopStateCall(node) {
    ...
    if (Core.isCallExpressionNode(node) && Core.isIdentifierWithName(node.object, "gpu_pop_state")) {
        return true;
    }
...
function isGpuStateCall(node, expectedName, { allowStatements = false } = {}) {
    ...
    } else if (node && Core.isCallExpressionNode(node)) {
        expression = node;
    }
...
function isVertexFormatEndCall(node) {
    return !!node && Core.isCallExpressionNode(node) && Core.isIdentifierWithName(node.object, "vertex_format_end");
}

function isVertexFormatBeginCall(node) {
    return !!node && Core.isCallExpressionNode(node) && Core.isIdentifierWithName(node.object, "vertex_format_begin");
}
...
export function isLiteralZero(node) {
    if (!node || node.type !== "Literal") {
        return false;
    }
...
export function isLiteralOne(node) {
    if (!node || node.type !== "Literal") {
        return false;
    }
...
export function isDrawFunctionCall(node) {
    if (!node || node.type !== "CallExpression") {
        return false;
    }
...
export function extractSurfaceTargetName(node) {
    if (!node || node.type !== "CallExpression") {
        return null;
    }
...
export function isNegativeOneLiteral(node) {
    if (!node || typeof node !== "object") {
        return false;
    }
    ...
    if (Core.isUnaryExpressionNode(node) && node.operator === "-" && node.prefix) {
        const argument = node.argument;

        if (!argument || !Core.isLiteralNode(argument)) {
            return false;
        }
