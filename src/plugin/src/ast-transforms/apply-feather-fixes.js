    ...
    for (const diagnostic of diagnostics) {
        ...
        if (diagnosticId === "GM2016") {
            registerFeatherFixer(registry, diagnosticId, () => ({ ast }) => {
                const fixes = localizeInstanceVariableAssignments({ ast, diagnostic });
...
function localizeInstanceVariableAssignments({ ast, diagnostic }) {
...
function convertAssignmentToLocalVariable({ node, parent, property, diagnostic, eventMarkers }) {
    ...
    const eventMarker = findEventMarkerForIndex(eventMarkers, getNodeStartIndex(node));
