...
function replaceFunctionArgumentReferencesWithNamedParameters({
    ast,
    diagnostic
}) {
    if (!diagnostic || !ast || typeof ast !== "object") {
    ...
    const visit = (node, parent, property) => {
        ...
        if (isFunctionLikeNode(node)) {
            const functionFixes = replaceArgumentReferencesInFunction(
                node,
                diagnostic
            );
...
function replaceArgumentReferencesInFunction(node, diagnostic) {
    ...
    const visit = (current, parent, property) => {
        ...
        (typeof property !== "string" && typeof property !== "number")
            ...
            const replacement = createIdentifierFromParameter(
