...
void test("preserves uninitialized globalvar declarations when project-safe rewrite is unavailable", async () => {
...
void test("rewrites uninitialized globalvar declarations when project-aware runtime allows undefined initialization", async () => {
    try {
        setSemanticSafetyRuntime({
...
void test("skips globalvar rewrites that require project-wide edits when refactor runtime reports cross-file usage", async () => {
    const reports: Array<SemanticSafetyReportRecord> = [];
...
void test("renames hoisted loop cache variables when local fallback detects collisions", async () => {
    const reports: Array<SemanticSafetyReportRecord> = [];
    const source = [
...
void test("uses project-aware occupancy checks when the runtime adapter is available", async () => {
    const reports: Array<SemanticSafetyReportRecord> = [];
    const source = [
        "function demo(value_list) {",
        "    for (var i = 0; i < array_length(value_list); i += 1) {",
    ...
    try {
        ...
        assert.ok(
            reports.some(
                (report) => report.code === "GML_SEMANTIC_SAFETY_LOOP_HOIST_RENAMED" && report.mode === "project-aware"
            ),
