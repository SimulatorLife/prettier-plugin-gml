...
describe("applyFeatherFixes transform", () => {
    ...
    it("converts GM2016 assignments to local declarations outside the Create event", () => {
        ...
        const [createAssignment, localizedVariable, additiveAssignment] = ast.body ?? [];
        ...
        const appliedDiagnostics = ast._appliedFeatherDiagnostics ?? [];
        const gm2016 = appliedDiagnostics.find((entry) => entry.id === "GM2016");

        assert.ok(gm2016, "Expected GM2016 metadata to be recorded on the AST.");
        assert.strictEqual(gm2016.automatic, true);
        assert.strictEqual(gm2016.target, "ammo");
        assert.ok(gm2016.range);

        const declarationDiagnostics = localizedVariable._appliedFeatherDiagnostics ?? [];
        assert.strictEqual(
