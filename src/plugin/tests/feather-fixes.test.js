...
describe("applyFeatherFixes transform", () => {
    ...
    it("inserts surface_reset_target after surface_set_target flagged by GM2005 and records metadata", () => {
        ...
        assert.ok(setTargetIndex >= 0, "Expected surface_set_target call in test fixture AST.");
        ...
        assert.ok(resetCallIndex > setTargetIndex, "Expected surface_reset_target call to be inserted after surface_set_target.");
        ...
        assert.strictEqual(resetDiagnostics.some((entry) => entry.id === "GM2005"), true);
    });
});
