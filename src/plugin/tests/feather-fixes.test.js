...
describe("applyFeatherFixes transform", () => {
    ...
    it("resets draw alpha flagged by GM2063 and records metadata", () => {
        const source = ["draw_set_alpha(0.5);", "", "draw_self();"].join("\n");
        ...
        const [setAlphaCall, resetCall, drawCall] = statements;

        assert.ok(setAlphaCall);
        assert.ok(resetCall);
        assert.ok(drawCall);
        assert.strictEqual(setAlphaCall.type, "CallExpression");
        assert.strictEqual(setAlphaCall.object?.name, "draw_set_alpha");
        assert.strictEqual(drawCall.type, "CallExpression");
        assert.strictEqual(drawCall.object?.name, "draw_self");
        ...
        assert.strictEqual(
            argument.value === "1" || argument.value === 1,
            true
        );

        const appliedDiagnostics = ast._appliedFeatherDiagnostics ?? [];
        const gm2063 = appliedDiagnostics.find(
            (entry) => entry.id === "GM2063"
        );

        assert.ok(
            gm2063,
            "Expected GM2063 metadata to be recorded on the AST."
        );
        assert.strictEqual(gm2063.automatic, true);
        assert.strictEqual(gm2063.target, "draw_set_alpha");
        assert.ok(gm2063.range);

        const resetMetadata = resetCall._appliedFeatherDiagnostics ?? [];
        assert.strictEqual(
            resetMetadata.some((entry) => entry.id === "GM2063"),
            true,
            "Expected GM2063 metadata to be recorded on the inserted reset call."
        );
    });
