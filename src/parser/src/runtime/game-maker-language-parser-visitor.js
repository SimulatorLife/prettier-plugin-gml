...
import { collectPrototypeMethodNames, collectVisitMethodNames, createWrapperSymbols, definePrototypeMethods, ensureHasInstancePatched } from "./parse-tree-helpers.js";
const DEFAULT_VISIT_CHILDREN_DELEGATE = ({ fallback }) => fallback();
const PARSE_TREE_VISITOR_PROTOTYPE = Object.getPrototypeOf(GameMakerLanguageParserVisitorBase.prototype);
const { instance: WRAPPER_INSTANCE_MARKER, patchFlag: HAS_INSTANCE_PATCHED_MARKER } = createWrapperSymbols("GameMakerLanguageParserVisitor");
...
definePrototypeMethods(GameMakerLanguageParserVisitor.prototype, INHERITED_METHOD_NAMES, (methodName) => {
    const inherited = PARSE_TREE_VISITOR_PROTOTYPE[methodName];
    return function (...args) {
        return inherited.apply(this, args);
    };
});
definePrototypeMethods(GameMakerLanguageParserVisitor.prototype, VISIT_METHOD_NAMES, (methodName) => function (ctx) {
    return this._visitUsingDelegate(methodName, ctx);
});
//# sourceMappingURL=game-maker-language-parser-visitor.js.map