...
import { isBinaryOperator } from "../ast/node-helpers.js";
...
function attemptCondenseNumericChainWithMultipleBases(node, helpers, context) {
    ...
    const coefficientIsMeaningful = Math.abs(coefficient - 1) > unitTolerance;
    const nonNumericLcm = nonNumericTerms.length;

    if (meaningfulNumericFactorCount >= 2 && coefficientIsMeaningful) {
        return false;
    }
    ...
    // Conservative: don't rewrite when coefficient yields an ugly rational
    // with small denominator. The plugin's implementation uses a number of
    // heuristics; keep behaviour aligned by refusing rewrite for small
    // coefficient denominators.
    const approxInt = toApproxInteger(coefficient);
    if (approxInt !== null && approxInt !== 1) {
        return false;
    }

    // If we get here, we have at least two non-numeric terms and numeric
    // factors in numerator and denominator; prefer to emit a simplified
    // multiplicative expression using common factors when safe.
    // For now conservatively avoid applying a rewrite here unless further
    // simplifications are implemented. Returning false preserves current
    // behaviour until a safer refactor is ported.
    return false;
}
