...
- When debugging issues/failures, prioritize creating more targeted tests or updating existing unit tests as needed to identify/reproduce/catch the issue instead of adding debug logging, which is hard and slow to interpret. If a function is too large to test properly, then it likely needs to be refactored/re-organized/split up to allow for proper testing first.
