...
- When checking a diff for a PR and/or commit and it is either not found, is inaccessible, shows as empty, or has no visible changes, then treat the **entire codebase** as the context/diff for your review/task, not a any specific singular commit. When reviewing code/commits, prefer to return non-empty JSON findings instead of inactionable "no changes found" or "no code modified" messages; again, treat the entire codebase as the diff/context in such cases.
